       ...
       if (isButton && isCmd && !quotedMsg.fromMe) return
        ...
        switch(command){
            ...
            case prefix+'menu': case prefix+'help':{
                ...
                xinz.sendButtonsLoc(from, `Hai Kak @${sender.split('@')[0]}\n\nSaya NekoBotz, Bot WhatsApp yg membantu kamu untuk mempermudah sesuatu seperti Membuat Sticker dan Lainnya, Ada Butuh Info Dariku?`, `Note: Kalo kamu pakai wa lama atau mod, dan button ga keliatan, langsung aja ketik ${prefix}allmenu`, qqppp, fs.readFileSync(setting.pathImg), [sender])
                }
                break
             case prefix+'allmenu':{
                    try {
                    ...
                    xinz.sendList(from, `Selamat ${ucap}`, `Hai kak @${sender.split('@')[0]}, pilih Menu NekoBotz disini`, `Jangan lupa Donasi ya Kak`,`Pilih Disini`, list, msg, [sender])
                })
            }
                break
//------------------< PRAY Command >-------------------
				...
				if (isQuotedMsg && args.length < 2) {
                    ...
                    .then((res) => {
                        if (res.participants[0][quotedMsg.sender.split("@")[0] + '@c.us'].code === "403"){
                            ...
                            xinz.sendMessage(quotedMsg.sender, { groupName: groupName, groupJid: from, inviteCode: au.invite_code, inviteExpiration: au.invite_code_exp, caption: `Invited By NekoBotz`, jpegThumbnail: pepeqq }, groupInviteMessage)
					...
					.then((res) => {
                        if (res.participants[0][args[1] + '@c.us'].code === "403"){
                            ...
                            xinz.sendMessage(args[1] + '@s.whatsapp.net', { groupName: groupName, groupJid: from, inviteCode: au.invite_code, inviteExpiration: au.invite_code_exp, caption: `Invited By NekoBotz`, jpegThumbnail: pepeqq }, groupInviteMessage)
                            reply(`Mengirimkan groupInvite kepada nomor tersebut`)
